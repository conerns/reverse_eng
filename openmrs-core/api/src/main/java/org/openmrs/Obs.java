 ...
 * <br>
 * Observations are collected and grouped together into one Encounter (one
 * visit). Obs can be
 * grouped in a hierarchical fashion. <br>
 * <br>
 * <p>
 * The {@link #getObsGroup()} method returns an optional parent. That parent
 * object is also an Obs.
 * The parent Obs object knows about its child objects through the
 * {@link #getGroupMembers()}
 * method.
 * </p>
 * <p>
 * (Multi-level hierarchies are achieved by an Obs parent object being a member
 * of another Obs
 * (grand)parent object) Read up on the obs table:
 * http://openmrs.org/wiki/Obs_Table_Primer In an
 * OpenMRS installation, there may be an occasion need to change an Obs.
 * </p>
 * <p>
 * For example, a site may decide to replace a concept in the dictionary with a
 * more specific set of
 * concepts. An observation is part of the official record of an encounter.
 * There may be legal,
 * ethical, and auditing consequences from altering a record. It is recommended
 * that you create a
...
public class Obs extends BaseFormRecordableOpenmrsData {
	...
	public enum Interpretation {
		NORMAL, ABNORMAL, CRITICALLY_ABNORMAL, NEGATIVE, POSITIVE, CRITICALLY_LOW, LOW, HIGH, CRITICALLY_HIGH,
		VERY_SUSCEPTIBLE, SUSCEPTIBLE, INTERMEDIATE, RESISTANT, SIGNIFICANT_CHANGE_DOWN, SIGNIFICANT_CHANGE_UP,
	 ...
	 * note:
	 * obsGroup.getConcept().isSet() should be true This will be non-null if this
	 * obs is a member of
	 * another groupedObs
	 * 
	 * @see #isObsGrouping() (??)
	...
	/**
	 * Required parameters constructor A value is also required, but that can be one
	 * of: valueCoded,
	...
	/**
	 * Get the concept description that is tied to the concept name that was used
	 * when making this
	 * observation
	 * 
	 ...
	 * org.openmrs.Concept#isSet()) If this is non-null, it means the current Obs is
	 * in the list
	 ...
	 * obsGroup.getGroupMembers().
	 * That must be done (and should be done) manually. (I am not doing it here for
	 * fear of screwing
	...
	/**
	 * Convenience method that checks for if this obs has 1 or more group members
	 * (either voided or
	 * non-voided) Note this method differs from hasGroupMembers(), as that method
	 * excludes voided
	 * obs; logic is that while a obs that has only voided group members should be
	 * seen as
	 ...
	 * confusion on names.
	 * However, jstl in a web layer (or any psuedo-getter) access isn't good with
	 * both an
	 * "isObsGroup" method and a "getObsGroup" method. Which one should be returned
	 * with a
	 * simplified jstl call like ${obs.obsGroup} ? With this setup,
	 * ${obs.obsGrouping} returns a
	 * boolean of whether this obs is a parent and has members. ${obs.obsGroup}
	 * returns the parent
	...
	/**
	 * A convenience method to check for nullity and length to determine if this obs
	 * has group
	...
	/**
	 * Convenience method that checks for nullity and length to determine if this
	 * obs has group
	 * members. The parameter specifies if this method whether or not voided obs
	 * should be
	 * considered.
	 * 
	 * @param includeVoided determines if Voided members should be considered as
	 *                      group members.
	 * @return true if this is the parent group of other Obs
	 *         <strong>Should</strong> return true if this obs has group members
	 *         based on parameter
	 */
	 ...
	 * <p>
	 * If it's not a group (i.e.
	 * {@link #getConcept()}.{@link org.openmrs.Concept#getSet()} is not
	...
	/**
	 * Get the group members of this obs group, if this obs is a group. This method
	 * will either
	 * return all group members, or only non-voided group members, depending on if
	 * the argument is
	 * set to be true or false respectively.
	 * 
	 * @param includeVoided
	 * @return the set of group members in this obs group
	 *         <strong>Should</strong> Get all group members if passed true, and
	 *         non-voided if passed false
	 */
	 ...
	 * @see #addGroupMember(Obs)
	 * @see #hasGroupMembers()
	 *      <strong>Should</strong> mark the obs as dirty when the set is changed
	 *      from null to a non empty one
	 *      <strong>Should</strong> not mark the obs as dirty when the set is
	 *      changed from null to an empty one
	 *      <strong>Should</strong> mark the obs as dirty when the set is replaced
	 *      with another with different members
	 *      <strong>Should</strong> not mark the obs as dirty when the set is
	 *      replaced with another with same members
	 */
	...
	/**
	 * Convenience method to add the given <code>obs</code> to this grouping. Will
	 * implicitly make
	 ...
	 *      <strong>Should</strong> return false when a duplicate obs is added as a
	 *      member
	 */
	...
	/**
	 * Convenience method that returns related Obs If the Obs argument is not an
	 * ObsGroup: a
	 * Set&lt;Obs&gt; will be returned containing all of the children of this Obs'
	 * parent that are
	 * not ObsGroups themselves. This will include this Obs by default, unless
	 * getObsGroup() returns
	 * null, in which case an empty set is returned. If the Obs argument is an
	 * ObsGroup: a
	 * Set&lt;Obs&gt; will be returned containing 1. all of this Obs' group members,
	 * and 2. all
	 ...
	 * <code>{@link #getPerson()}.getPersonId()</code>. It is duplicated here for
	 * speed and
	 * simplicity reasons
	 * 
	...
	/**
	 * Set the person object to this obs object. This will also set the personId on
	 * this obs object
	 * 
	...
	/**
	 * Sets the value of this obs to the specified valueBoolean if this obs has a
	 * boolean concept.
	 * 
	 * @param valueBoolean the boolean value matching the boolean coded concept to
	 *                     set to
	 */
	 ...
	 *         <strong>Should</strong> return true for value_numeric concepts if
	 *         value is 1
	 *         <strong>Should</strong> return false for value_numeric concepts if
	 *         value is 0
	 *         <strong>Should</strong> return null for value_numeric concepts if
	 *         value is neither 1 nor 0
	 */
	public Boolean getValueAsBoolean() {
		...
		// returning null is preferred to defaulting to false to support validation of
		// user input is from a form
		return null;
	}
	 ...
	 *         <strong>Should</strong> return true if value coded answer concept is
	 *         true concept
	 *         <strong>Should</strong> return false if value coded answer concept is
	 *         false concept
	 */
	...
	/**
	 * @return the value of this obs as a Date. Note that this uses a
	 *         java.util.Date, so it includes
	...
	/**
	 * @return the time value of this obs. Note that this uses a java.util.Date, so
	 *         it includes a
	 ...
	 * ComplexObsHandler. The
	 * valueComplex has two parts separated by a bar '|' character: part A) the
	 * title; and part B)
	 * the URI. The title is the readable description of the valueComplex that is
	 * returned by
	 * {@link Obs#getValueAsString(java.util.Locale)}. The URI is the location where
	 * the ComplexData is stored.
	 * 
	 * @return readable title and URI for the location of the ComplexData binary
	 *         object.
	 * @since 1.5
	 */
	 ...
	 * the ComplexData is stored.
	 * 
	 * @param valueComplex readable title and URI for the location of the
	 *                     ComplexData binary object.
	 * @since 1.5
	 */
	 ...
	 * <br>
	 * {@link ComplexObsHandler}s that are registered to {@link ConceptComplex}s
	 * will persist the
	 * {@link ComplexData#getData()} object to the correct place for the given
	 * concept.
	 * 
	 * @param complexData
	 * @since 1.5
	...
	/**
	 * Get the ComplexData. This is retrieved by the {@link ComplexObsHandler} from
	 * the file system
	...
	/**
	 * Convenience method for obtaining the observation's value as a string If the
	 * Obs is complex,
	 * returns the title of the complexData denoted by the section of
	 * getValueComplex() before the
	 * first bar '|' character; or returns the entire getValueComplex() if the bar
	 * '|' character is
	 * missing.
	 *
	 * @param locale locale for locale-specific depictions of value
	 *               <strong>Should</strong> return first part of valueComplex for
	 *               complex obs
	 *               <strong>Should</strong> return first part of valueComplex for
	 *               non null valueComplexes
	 *               <strong>Should</strong> return non precise values for
	 *               NumericConcepts
	 *               <strong>Should</strong> return date in correct format
	 *               <strong>Should</strong> not return long decimal numbers as
	 *               scientific notation
	 *               <strong>Should</strong> use commas or decimal places depending
	 *               on locale
	 *               <strong>Should</strong> not use thousand separator
	 *               <strong>Should</strong> return regular number for size of zero
	 *               to or greater than ten digits
	 *               <strong>Should</strong> return regular number if decimal places
	 *               are as high as six
	 */
	...
	/**
	 * Sets the value for the obs from a string depending on the datatype of the
	 * question concept
	 *
	 * @param s the string to coerce to a boolean
	 *          <strong>Should</strong> set value as boolean if the datatype of the
	 *          question concept is boolean
	...
	/**
	 * When ObsService updates an obs, it voids the old version, creates a new Obs
	 * with the updates,
	 * and adds a reference to the previousVersion in the new Obs.
	 * getPreviousVersion returns the
	...
	/**
	 * Returns true if any change has been made to an Obs instance. In general, the
	 * only time
	 * isDirty() is going to return false is when a new Obs has just been
	 ...
	 *        <strong>Should</strong> return true when any immutable field has been
	 *        changed
	 *        <strong>Should</strong> return false when only mutable fields are
	 *        changed
	 *        <strong>Should</strong> return true when an immutable field is changed
	 *        from a null to a non null value
	 *        <strong>Should</strong> return true when an immutable field is changed
	...
	/**
	 * Similar to FHIR's Observation.interpretation. Supports a subset of FHIR's
	 * Observation
	 * Interpretation Codes. See
	...
	/**
	 * Similar to FHIR's Observation.status. Supports a subset of FHIR's
	 * ObservationStatus values.
	 * At present OpenMRS does not support FHIR's REGISTERED and CANCELLED statuses,
	 * because we
