...
/**
 * Contains methods pertaining to creating/updating/deleting/retiring Concepts,
 * Drugs, Concept
...
public interface ConceptService extends OpenmrsService {
	 ...
	 *         <strong>Should</strong> return null if no object found with given
	 *         uuid
	 */
	...
	/**
	 * Save or update the given <code>Concept</code> or <code>ConceptNumeric</code>
	 * in the database
	 * <p>
	 * If this is a new concept, the returned concept will have a new
	 * {@link Concept#getConceptId()}
	 * inserted into it that was generated by the database
	 * 
	 * @param concept The <code>Concept</code> or <code>ConceptNumeric</code> to
	 *                save or update
	 * @return the <code>Concept</code> or <code>ConceptNumeric</code> that was
	 *         saved or updated
	 * @throws APIException
	 * @throws ConceptsLockedException
	 * @throws ConceptInUseException
	 *                                 <strong>Should</strong> put generated concept
	 *                                 id onto returned concept
	 *                                 <strong>Should</strong> create new concept in
	 *                                 database
	 *                                 <strong>Should</strong> update concept
	 *                                 already existing in database
	 *                                 <strong>Should</strong> generate id for new
	 *                                 concept if none is specified
	 *                                 <strong>Should</strong> keep id for new
	 *                                 concept if one is specified
	 *                                 <strong>Should</strong> save non
	 *                                 ConceptNumeric object as conceptNumeric
	 *                                 <strong>Should</strong> save non
	 *                                 ConceptComplex object as conceptComplex
	 *                                 <strong>Should</strong> save changes between
	 *                                 concept numeric and complex
	 *                                 <strong>Should</strong> save a ConceptNumeric
	 *                                 as a concept
	 *                                 <strong>Should</strong> save a new
	 *                                 ConceptNumeric
	 *                                 <strong>Should</strong> void the conceptName
	 *                                 if the text of the name has changed
	 *                                 <strong>Should</strong> create a new
	 *                                 conceptName when the old name is changed
	 *                                 <strong>Should</strong> set a preferred name
	 *                                 for each locale if none is marked
	 *                                 <strong>Should</strong> not fail when a
	 *                                 duplicate name is edited to a unique value
	 *                                 <strong>Should</strong> create a reference
	 *                                 term for a concept mapping on the fly when
	 *                                 editing a concept
	 *                                 <strong>Should</strong> create a reference
	 *                                 term for a concept mapping on the fly when
	 *                                 creating a concept
	 *                                 <strong>Should</strong> add new concept name
	 *                                 <strong>Should</strong> not set audit info if
	 *                                 the concept is not edited
	 *                                 <strong>Should</strong> set audit info if an
	 *                                 item is removed from any of its child
	 *                                 collections
	 *                                 <strong>Should</strong> set audit info if any
	 *                                 item in the child collections is edited
	 *                                 <strong>Should</strong> set audit info if an
	 *                                 item is added to any of its child collections
	 *                                 <strong>Should</strong> pass when saving a
	 *                                 concept after removing a name
	 *                                 <strong>Should</strong> save a conceptNumeric
	 *                                 with allowDecimal value
	 */
	@Authorized({ PrivilegeConstants.MANAGE_CONCEPTS })
	public Concept saveConcept(Concept concept) throws APIException;

	/**
	 * Save or update the given <code>Drug</code> in the database. If this is a new
	 * drug, the
	 * returned drug object will have a new {@link Drug#getDrugId()} inserted into
	 * it that was
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> put generated drug id onto
	 *                      returned drug
	 *                      <strong>Should</strong> create new drug in database
	 *                      <strong>Should</strong> update drug already existing in
	 *                      database
	 */
	@Authorized({ PrivilegeConstants.MANAGE_CONCEPTS })
	...
	/**
	 * Completely purge a <code>Concept</code> or <code>ConceptNumeric</code> from
	 * the database.
	 * This should not typically be used unless desperately needed. Most should just
	 * be retired. See
	 * {@link #retireConcept(Concept, String)}
	 * 
	 * @param conceptOrConceptNumeric The <code>Concept</code> or
	 *                                <code>ConceptNumeric</code> to
	 *                                remove from the system
	 * @throws APIException
	 *                      <strong>Should</strong> purge the concept if not being
	 *                      used by an obs
	 *                      <strong>Should</strong> fail if any of the conceptNames
	 *                      of the concept is being used by an obs
	 */
	 ...
	 *                      <strong>Should</strong> not change attributes of drug
	 *                      that is already not retired
	 */
	@Authorized(PrivilegeConstants.MANAGE_CONCEPTS)
	public Drug unretireDrug(Drug drug) throws APIException;

	/**
	 * Completely purge a Drug from the database. This should not typically be used
	 * unless
	 ...
	 * 
	 * @param sortBy         The property name to sort by; if null or invalid,
	 *                       concept_id is used.
	 * @param asc            true = sort ascending; false = sort descending
	 * @param includeRetired If <code>true</code>, retired concepts will also be
	 *                       returned
	 * @return a List&lt;Concept&gt; object containing all of the sorted concepts
	 * @throws APIException
	 *                      <strong>Should</strong> order by concept id and include
	 *                      retired when given no parameters
	 *                      <strong>Should</strong> order by concept id descending
	 *                      when set asc parameter to false
	 *                      <strong>Should</strong> exclude retired concepts when
	 *                      set includeRetired to false
	...
	/**
	 * Returns a list of concepts matching any part of a concept name, this method
	 * is case
	 ...
	 *         <strong>Should</strong> pass irrespective of the case of the passed
	 *         parameter
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	 ...
	 *         <strong>Should</strong> find concepts with names in more specific
	 *         locales
	 *         <strong>Should</strong> find concepts with names in more generic
	 *         locales
	 *         <strong>Should</strong> find concepts with names in same specific
	 *         locale
	 ...
	 *                      <strong>Should</strong> return null if no matching drug
	 *                      is found
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	...
	/**
	 * Get drugs by concept. This method is the utility method that should be used
	 * to generically
	 * retrieve all Drugs in the system.
	 * 
	 * @param includeRetired If <code>true</code> then the search will include
	 *                       voided Drugs
	 * @return A List&lt;Drug&gt; object containing all matching Drugs
	 *         <strong>Should</strong> return all drugs including retired ones if
	 *         given true
	 *         <strong>Should</strong> return all drugs excluding retired ones if
	 *         given false
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> not fail due to no name in
	 *                      search
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	 ...
	 * @throws APIException
	 * @return List&lt;ConceptClass&gt; object with all ConceptClass objects
	 *         <strong>Should</strong> return all concept classes including retired
	 *         ones when given true
	 *         <strong>Should</strong> return all concept classes excluding retired
	 *         ones when given false
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPT_CLASSES)
	 ...
	 * @throws APIException
	 * @since 1.10
	 *        <strong>Should</strong> delete the specified conceptNameTag from the
	 *        database
	 */
	 ...
	 * @throws APIException
	 * @return List of ConceptDatatypes
	 *         <strong>Should</strong> return all concept datatypes including
	 *         retired ones when given true
	 *         <strong>Should</strong> return all concept datatypes excluding
	 *         retired ones when given false
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPT_DATATYPES)
	 ...
	 * getConceptSets(getConcept("ANTIRETROVIRAL MEDICATIONS")) and then take the
	 * conceptIds from
	 * the resulting list.
	 * 
	 ...
	 * 
	 * @param concept The concept representing the concept set
	 * @return A List&lt;Concept&gt; object containing all objects within the
	 *         ConceptSet
	 * @throws APIException
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	 ...
	 *         <strong>Should</strong> give a list of ConceptSet containing the
	 *         given Concept
	 *         <strong>Should</strong> give an empty list if no matching ConceptSet
	 *         is found
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> return all concept proposals
	 *                      including retired ones when given true
	 *                      <strong>Should</strong> return all concept proposals
	 *                      excluding retired ones when given false
	 */
	 ...
	 * 
	 * @param text
	 * @return a List&lt;ConceptProposal&gt; object containing matching concept
	 *         proposals
	 * @throws APIException
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPT_PROPOSALS)
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> not require mapped concept on
	 *                      reject action
	 *                      <strong>Should</strong> allow rejecting proposals
	 *                      <strong>Should</strong> throw APIException when mapping
	 *                      to null concept
	 */
	@Authorized(PrivilegeConstants.MANAGE_CONCEPTS)
	 ...
	 *                      <strong>Should</strong> throw APIException when mapping
	 *                      to null concept
	 *                      <strong>Should</strong> set value coded name when add
	 *                      synonym is selected
	 *                      <strong>Should</strong> not set value coded name when
	 *                      add concept is selected
	 *                      <strong>Should</strong> fail when adding a duplicate
	 *                      synonym
	 */
	@Authorized(PrivilegeConstants.MANAGE_CONCEPTS)
	 ...
	 * To navigate in
	 * the other direction, i.e., from Concept to its answers use
	 * Concept.getAnswers()
	 * 
	 * @param concept
	 * @return A List&lt;Concept&gt; containing all possible questions to which this
	 *         concept is a
	 *         valued_Coded answer
	 * @throws APIException
	 *                      <strong>Should</strong> return an empty list if concept
	 *                      id is null
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	...
	/**
	 * Finds the previous concept in the dictionary that has the next lowest concept
	 * id
	 * 
	 * @param concept the offset Concept
	 * @return the foundConcept
	 * @throws APIException
	 *                      <strong>Should</strong> return the concept previous to
	 *                      the given concept
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	 ...
	 * @return the foundConcept
	 * @throws APIException
	 *                      <strong>Should</strong> return the concept next to the
	 *                      given concept
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	 ...
	 * 
	 * @return A List&lt;Concept&gt; object of all concepts that occur as a
	 *         Drug.concept.
	 * @throws APIException
	 *                      <strong>Should</strong> give a list of all matching
	 *                      concepts
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	...
	/**
	 * Return a list of concept sources currently in the database Whether or not to
	 * return retired
	 ...
	 *         <strong>Should</strong> return all concept sources including retired
	 *         ones when given true
	 *         <strong>Should</strong> return all concept sources excluding retired
	 *         ones when given false
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPT_SOURCES)
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> not set creator if one is
	 *                      supplied already
	 *                      <strong>Should</strong> not set date created if one is
	 *                      supplied already
	 *                      <strong>Should</strong> save a ConceptSource with a null
	 *                      hl7Code
	 *                      <strong>Should</strong> not save a ConceptSource if
	 *                      voided is null
	 */
	...
	/**
	 * This effectively removes a concept source from the database. The source can
	 * still be
	 * referenced by old data, but no new data should use this source.
	 * 
	 * @param cs     the concept source to retire
	 * @param reason why the concept source is to be retired, must not be empty of
	 *               null
	 ...
	 *         <strong>Should</strong> not save a concept name tag if tag is null,
	 *         empty or whitespace
	...
	/**
	 * Looks up a concept via {@link ConceptMap} This will return the
	 * {@link Concept} which contains
	 * a {@link ConceptMap} entry whose <code>sourceCode</code> is equal to the
	 * passed
	 * <code>conceptCode</code> and whose {@link ConceptSource} has either a
	 * <code>name</code> or
	 ...
	 * 
	 * @param code       the code associated with a concept within a given
	 *                   {@link ConceptSource}
	 * @param sourceName the name or hl7Code of the {@link ConceptSource} to check
	 * @return the {@link Concept} that has the given mapping, or null if no
	 *         {@link Concept} found
	 * @throws APIException
	 *                      <strong>Should</strong> get concept with given code and
	 *                      and source hl7 code
	 *                      <strong>Should</strong> get concept with given code and
	 *                      source name
	 *                      <strong>Should</strong> return null if source code does
	 *                      not exist
	 *                      <strong>Should</strong> return null if mapping does not
	 *                      exist
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	 ...
	 * <code>hl7Code</code> that is equal to the passed <code>mappingCode</code> .
	 * Operates under
	 * the assumption that each mappingCode in a {@link ConceptSource} references
	 * one and only one
	 * non-retired {@link Concept}: if the underlying dao method returns more than
	 * one non-retired
	 * concept, this method will throw an exception; if the underlying dao method
	 * returns more than
	 * one concept, but only one non-retired concept, this method will return the
	 * non-retired
	 * concept; if the dao only returns retired concepts, this method will simply
	 * return the first
	 * concept in the list returns by the dao method; retired concepts can be
	 * excluded by setting
	 * the includeRetired parameter to false, but the above logic still applies
	 *
	 * @param code           the code associated with a concept within a given
	 *                       {@link ConceptSource}
	 * @param sourceName     the name or hl7Code of the {@link ConceptSource} to
	 *                       check
	 ...
	 * concepts
	 * {@link Concept}s which contain a {@link ConceptMap} entry whose
	 * <code>sourceCode</code> is
	 * equal to the passed <code>conceptCode</code> and whose {@link ConceptSource}
	 * has either a
	 ...
	 *                   {@link ConceptSource}
	 * @param sourceName the name or hl7Code of the {@link ConceptSource} to check
	 * @return the list of non-voided {@link Concept}s that has the given mapping,
	 *         or null if no
	 *         {@link Concept} found
	 * @throws APIException if the specified source+code maps to more than one
	 *                      concept
	 *                      <strong>Should</strong> get concepts with given code and
	 *                      and source hl7 code
	 *                      <strong>Should</strong> get concepts with given code and
	 *                      source name
	 *                      <strong>Should</strong> return empty list if source code
	 *                      does not exist
	 *                      <strong>Should</strong> return empty list if mapping
	 *                      does not exist
	...
	/**
	 * Looks up a concept via {@link ConceptMap} This will return the list of
	 * {@link Concept}s which
	 * contain a {@link ConceptMap} entry whose <code>sourceCode</code> is equal to
	 * the passed
	 * <code>conceptCode</code> and whose {@link ConceptSource} has either a
	 * <code>name</code> or
	 * <code>hl7Code</code> that is equal to the passed <code>mappingCode</code>.
	 ...
	 * @return ConceptSource
	 * @throws APIException
	 *                      <strong>Should</strong> get ConceptSource with the given
	 *                      name
	 *                      <strong>Should</strong> return null if no ConceptSource
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> get concept source with the
	 *                      given unique id
	 *                      <strong>Should</strong> return null if no concept source
	 *                      with given unique id is found
	 *                      <strong>Should</strong> return null if given an empty
	 *                      string
	...
	/**
	 * Checks if there are any observations (including voided observations) for a
	 * concept.
	 * 
	...
	/**
	 * Changes the datatype of a concept from boolean to coded when it has
	 * observations it is
	 * associated to.
	 * 
	 * @param conceptToChange the concept which to change
	 * @throws APIException
	 *                      <strong>Should</strong> convert the datatype of a
	 *                      boolean concept to coded
	 *                      <strong>Should</strong> fail if the datatype of the
	 *                      concept is not boolean
	 *                      <strong>Should</strong> explicitly add true concept as a
	 *                      value_Coded answer
	 *                      <strong>Should</strong> explicitly add false concept as
	 *                      a value_Coded answer
	 */
	@Authorized({ PrivilegeConstants.MANAGE_CONCEPTS })
	public void convertBooleanConceptToCoded(Concept conceptToChange) throws APIException;

	/**
	 * Checks if there are any observations (including voided observations) using a
	 * conceptName.
	 * 
	 * @param conceptName which is used or not used by an observation
	 * @return boolean true if the conceptName is used by an observation otherwise
	 *         false
	 * @throws APIException
	 * @since Version 1.7
	 */
	 ...
	 * 
	 * @param phrase           matched to the start of any word in any of the names
	 *                         of a concept (if
	 ...
	 * @param size             if non zero, all results after <code>start</code> +
	 *                         <code>size</code> will be
	 *                         removed
	 * @return a list of conceptSearchResults
	 * @throws APIException
	 *                      <strong>Should</strong> return concept search results
	 *                      that match unique concepts
	 *                      <strong>Should</strong> return a search result whose
	 *                      concept name contains all word tokens as first
	 *                      <strong>Should</strong> return a search result for
	 *                      phrase with stop words
	 *                      <strong>Should</strong> not return concepts with
	 *                      matching names that are voided
	 ...
	 * @return a list of conceptSearchResults
	 * @throws APIException
	 *                      <strong>Should</strong> return a list of all matching
	...
	/**
	 * Iterates over the words in names and synonyms (for each locale) and updates
	 * the concept
	 * index, note that this only updates the index of the specified concept. Use
	 * {@link ConceptService#updateConceptIndexes()} if you wish to update the
	 * entire concept index.
	 * 
	 ...
	 * 
	 * @param phrase         the string to search against (if blank/null, matches
	 *                       all concepts)
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> give a list of
	 *                      ConceptSearchResult for the matching Concepts
	 * @since 1.8
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	...
	/**
	 * Return the number of concepts matching a search phrase and the specified
	 * arguments
	 * 
	 * @param phrase           matched to the start of any word in any of the names
	 *                         of a concept
	 ...
	 * @param searchOnPhrase         Specifies if the search should match names
	 *                               starting with or contain the
	 *                               text
	 * @param searchDrugConceptNames Specifies whether a search on concept names for
	 *                               the drug's
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> return the total number of
	 *                      matching drugs
	 * @since 1.8
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	public Integer getCountOfDrugs(String drugName, Concept concept, boolean searchOnPhrase,
			boolean searchDrugConceptNames,
			boolean includeRetired) throws APIException;

	/**
	 * Returns a list of drugs with matching names or concept drug names and returns
	 * a specific
	 * number of them from the specified starting position. If start and length are
	 * not specified,
	 * then all matches are returned
	 * 
	 * @param drugName               the name of the drug
	 * @param concept                the drug concept
	 * @param searchKeywords         (since 1.11) Specifies whether the search
	 *                               should match keywords or just
	 *                               phrase
	 * @param searchDrugConceptNames Specifies whether a search on concept names for
	 *                               the drug's
	 *                               concept should be done or not
	 ...
	 * 
	 * @param locale The locale in which to search for the
	 *               <code>ConceptStopWord</code>
	 * @return list of concept stop words for given locale
	 *         <strong>Should</strong> return list of concept stop words for given
	 *         locale
	 *         <strong>Should</strong> return empty list if no stop words are found
	 *         for the given locale
	 *         <strong>Should</strong> return default Locale
	 *         <code>ConceptStopWord</code> if Locale is null
	 * @since 1.8
	 */
	 ...
	 * <p>
	 * If this is a new concept stop word, the returned concept stop word will have
	 * a new
	 * {@link org.openmrs.ConceptStopWord#getConceptStopWordId()} inserted into it
	 * that was
	 * generated by the database
	 * </p>
	 * 
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> generated concept stop word id
	 *                      onto returned concept stop word
	 *                      <strong>Should</strong> save concept stop word into
	 *                      database
	 *                      <strong>Should</strong> assign default Locale
	 *                      <strong>Should</strong> save concept stop word in
	 *                      uppercase
	 *                      <strong>Should</strong> fail if a duplicate
	 *                      conceptStopWord in a locale is added
	 * @since 1.8
	 */
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> delete the given concept stop
	...
	/**
	 * Gets drugs by the given ingredient, which can be either the drug itself or
	 * any ingredient.
	 * 
	...
	/**
	 * Returns a list of concept map types currently in the database excluding
	 * hidden ones
	 * 
	 * @return List of concept map type objects
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> return all the concept map types
	...
	/**
	 * Returns a list of concept map types currently in the database including or
	 * excluding retired
	 * and hidden ones as specified by the includeRetired and includeHidden
	 * arguments
	 * 
	 * @param includeRetired specifies if retired concept map types should be
	 *                       included
	 * @return List of concept map type objects
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> return all the concept map types
	 *                      if includeRetired and hidden are set to true
	 *                      <strong>Should</strong> return only un retired concept
	 *                      map types if includeRetired is set to false
	 *                      <strong>Should</strong> not include hidden concept map
	 *                      types if includeHidden is set to false
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPT_MAP_TYPES)
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> return a conceptMapType matching
	 *                      the specified uuid
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPT_MAP_TYPES)
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> add the specified concept map
	 *                      type to the database and assign to it an id
	 *                      <strong>Should</strong> update an existing concept map
	 *                      type
	 */
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> retire the specified
	 *                      conceptMapType with the given retire reason
	 *                      <strong>Should</strong> should set the default retire
	 *                      reason if none is given
	 */
	@Authorized(PrivilegeConstants.MANAGE_CONCEPT_MAP_TYPES)
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> unretire the specified concept
	 *                      map type and drop all retire related fields
	 */
	@Authorized(PrivilegeConstants.MANAGE_CONCEPT_MAP_TYPES)
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> delete the specified
	 *                      conceptMapType from the database
	 */
	...
	/**
	 * Returns a list of mappings from concepts to terms in the given reference
	 * terminology
	 * 
	 * @param conceptSource
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> return a List of ConceptMaps
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> return all concept reference
	 ...
	 * 
	 * @param includeRetired specifies if retired concept reference terms should be
	 *                       included
	 * @return a list of concept reference terms
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> return all the concept reference
	 *                      terms if includeRetired is set to true
	 *                      <strong>Should</strong> return only un retired concept
	 *                      reference terms if includeRetired is set to false
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPT_REFERENCE_TERMS)
	 ...
	 * @return the concept reference term object with the given concept reference
	 *         term id
	 * @since 1.9
	 * @throws APIException
	 */
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> return the concept reference
	 *                      term that matches the given uuid
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPT_REFERENCE_TERMS)
	public ConceptReferenceTerm getConceptReferenceTermByUuid(String uuid) throws APIException;

	/**
	 * Gets a concept reference term with the specified name from the specified
	 * concept source
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> return a concept reference term
	 *                      that matches the given name from the given source
	 *                      <strong>Should</strong> be case insensitive
	 *                      <strong>Should</strong> return null if no concept
	 *                      reference term is found
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPT_REFERENCE_TERMS)
	...
	/**
	 * Gets a concept reference term with the specified code from the specified
	 * concept source
	 * 
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> return a concept reference term
	 *                      that matches the given code from the given source
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPT_REFERENCE_TERMS)
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> add a concept reference term to
	 *                      the database and assign an id to it
	 *                      <strong>Should</strong> update changes to the concept
	 *                      reference term in the database
	 */
	 ...
	 * @param retireReason         the reason why the concept reference term is
	 *                             being retired
	 * @return the retired concept reference term object
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> retire the specified concept
	 *                      reference term with the given retire reason
	 *                      <strong>Should</strong> should set the default retire
	 *                      reason if none is given
	 */
	@Authorized(PrivilegeConstants.MANAGE_CONCEPT_REFERENCE_TERMS)
	public ConceptReferenceTerm retireConceptReferenceTerm(ConceptReferenceTerm conceptReferenceTerm,
			String retireReason)
			throws APIException;
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> unretire the specified concept
	 *                      reference term and drop all retire related fields
	 */
	@Authorized(PrivilegeConstants.MANAGE_CONCEPT_REFERENCE_TERMS)
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> purge the given concept
	 *                      reference term
	 *                      <strong>Should</strong> fail if given concept reference
	 *                      term is in use
	 */
	...
	/**
	 * Finds the concept reference term in the database that have a code or name
	 * that contains the
	 * specified search phrase.
	 * 
	 * @param query          the string to match against the reference term names or
	 *                       codes
	 * @param conceptSource  the concept source from which the terms should be
	 *                       looked up
	 * @param start          beginning index for the batch
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> return unique terms with a code
	 *                      or name containing the search phrase
	 *                      <strong>Should</strong> return only the concept
	 *                      reference terms from the given concept source
	 */
	@Authorized({ PrivilegeConstants.GET_CONCEPT_REFERENCE_TERMS })
	...
	/**
	 * Returns the count of concept reference terms that match the specified
	 * arguments
	 * 
	 ...
	 * @since 1.9
	 * @throws APIException
	 *                      <strong>Should</strong> include retired terms if
	 *                      includeRetired is set to true
	 *                      <strong>Should</strong> not include retired terms if
	 *                      includeRetired is set to false
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPT_REFERENCE_TERMS)
	...
	/**
	 * Fetches all the {@link ConceptReferenceTermMap} where the specified reference
	 * term is the
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> return all concept reference
	 *                      term maps where the specified term is the termB
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPT_REFERENCE_TERMS)
	 ...
	 * <p>
	 * This method is case insensitive. It searches for exactly matching names and
	 * close matching
	 * locales (if exactLocale = false). It considers only non-voided names and all
	 * concepts.
	 * 
	 * @param name
	 * @param locale      <code>null</code> = all locales
	 * @param exactLocale <code>false</code> if search for both global and country
	 *                    specific,
	 *                    <code>true</code> if <code>null</code>
	 * @return the list of concepts
	 * @throws APIException
	 * @since 1.9, 1.8.4
	 *        <strong>Should</strong> return concepts for all countries and global
	 *        language given language only locale
	 *        <strong>Should</strong> return concepts for specific country and
	 *        global language given language and country
	 *        locale
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	 ...
	 * <p>
	 * Concept name is considered duplicate if it is a default non-retired name for
	 * a non-voided
	...
	/**
	 * Fetches un retired drugs that match the specified search phrase. The logic
	 * matches on drug
	 * names, concept names of the associated concepts or the concept reference term
	 * codes of the
	 * drug reference term mappings
	 * 
	 * @param searchPhrase   The string to match against
	 * @param locale         The locale to match against when searching in drug
	 *                       concept names
	 * @param exactLocale    If false then concepts with names in a broader locale
	 *                       will be matched e.g
	 *                       in case en_GB is passed in then en will be matched
	 * @param includeRetired Specifies if retired drugs that match should be
	 *                       included or not
	 * @return A list of matching drugs
	 * @since 1.10
	 *        <strong>Should</strong> get drugs with names matching the search
	 *        phrase
	 *        <strong>Should</strong> include retired drugs if includeRetired is set
	 *        to true
	 *        <strong>Should</strong> get drugs linked to concepts with names that
	 *        match the phrase
	 *        <strong>Should</strong> get drugs linked to concepts with names that
	 *        match the phrase and locale
	 *        <strong>Should</strong> get drugs linked to concepts with names that
	 *        match the phrase and related locales
	 *        <strong>Should</strong> get drugs that have mappings with reference
	 *        term codes that match the phrase
	 *        <strong>Should</strong> return unique drugs
	 *        <strong>Should</strong> return all drugs with a matching term code or
	 *        drug name or concept name
	...
	/**
	 * Fetches all drugs with reference mappings to the specified concept source
	 * that match the
	 ...
	 * @param includeRetired      specifies if retired drugs should be included or
	 *                            not
	 * @since 1.10
	 * @return the list of {@link Drug}
	 * @throws APIException
	 *                      <strong>Should</strong> get a list of all drugs that
	 *                      match on all the parameter values
	 *                      <strong>Should</strong> return retired and non-retired
	 *                      drugs if includeRetired is set to true
	 *                      <strong>Should</strong> return empty list if no matches
	 *                      are found
	 ...
	 *                      <strong>Should</strong> fail if no code and concept
	 *                      source and withAnyOfTheseTypes are provided
	...
	/**
	 * Gets the "best" matching drug, i.e. matching the earliest ConceptMapType
	 * passed in e.g.
	 * getDrugByMapping("12345", rxNorm, Arrays.asList(sameAs, narrowerThan)) If
	 * there are multiple
	 * matches for the highest-priority ConceptMapType, throw an exception
	 * 
	 * @param code                                   the code the reference term
	 *                                               code to match on
	 ...
	 * @throws APIException
	 *                      <strong>Should</strong> return a drug that matches the
	 *                      code and source
	 *                      <strong>Should</strong> return a drug that matches the
	 *                      code and source and the best map type
	 *                      <strong>Should</strong> fail if multiple drugs are found
	 *                      matching the best map type
	...
	/**
	 * An Orderable concept is one where its conceptClass has a mapping in the
	 * order_type_class_map
	 * table. This method searches for orderable concepts which match the specified
	 * arguments
	 * 
	 ...
	 *        <strong>Should</strong> return an empty list if no concept search
	 *        result is found
	 */
	 ...
	 * @since 2.0
	 *        <strong>Should</strong> return all concept attribute types including
	 *        retired ones
	 */
	 ...
	 * @since 2.0
	 *        <strong>Should</strong> return the concept attribute type with the
	 *        given id
	 *        <strong>Should</strong> return null if no concept attribute type
	...
	/**
	 * Find concept attribute types matching the given parameters. Retired types are
	 * included in the
	 * results
	 *
	 * @param name (optional) The name of type
	 * @return list of ConceptAttributeTypes that matches <em>name</em> partially or
	 *         completely
	 * @since 2.0
	 * @throws APIException
	 *                      <strong>Should</strong> return concept attribute types
	 *                      performing fuzzy match on given name
	 *                      <strong>Should</strong> return empty list when no
	 *                      concept attribute types match given name
	 */
	@Authorized({ PrivilegeConstants.GET_CONCEPT_ATTRIBUTE_TYPES })
	 ...
	 *        <strong>Should</strong> return null if no concept attribute type
	 *        exists with the exact specified name
	 */
	@Authorized({ PrivilegeConstants.GET_CONCEPT_ATTRIBUTE_TYPES })
	 ...
	 *        <strong>Should</strong> return null if no concept attribute has the
	 *        given uuid
	 */
	@Authorized(PrivilegeConstants.GET_CONCEPTS)
	...
	/**
	 * @param conceptAttributeType
	 * @since 2.0 Checks if there are any concept attributes (including voided
	 *        attributes) for a
