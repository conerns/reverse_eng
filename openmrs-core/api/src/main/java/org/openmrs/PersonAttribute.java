 ...
 * information about a
 * user/patient to their database. PersonAttributes are essentially just
 * key-value pairs. However,
 * the PersonAttributeType can be defined in such a way that the value portion
 * of this
 * PersonAttribute is a foreign key to another database table (like to the
 * location table, or
 * concept table). This gives a PersonAttribute the ability to link to any other
 * part of the
...
public class PersonAttribute extends BaseChangeableOpenmrsData
	 ...
	 * 
	 * @param target a PersonAttribute that will have the state of <code>this</code>
	 *               copied into it
	 ...
	 * attribute for
	 * equality. Note: Null/empty fields on <code>otherAttribute</code> /will not/
	 * cause a false
	 ...
	 *         <strong>Should</strong> return true if attributeType value and void
	 *         status are the same
	 */
	 ...
	 * implements
	 * <code>Attributable</code>, hydrate(value) is called. Defaults to just
	 * returning getValue()
	 * 
	 * @return hydrated object or getValue()
	 *         <strong>Should</strong> load class in format property
	 *         <strong>Should</strong> still load class in format property if not
	 *         Attributable
	 */
	@SuppressWarnings("unchecked")
	 ...
	 *      <strong>Should</strong> return negative if other attribute has earlier
	 *      date created
	 *      <strong>Should</strong> return negative if this attribute has lower
	 *      attribute type than argument
	 *      <strong>Should</strong> return negative if other attribute has lower
	 *      value
	 *      <strong>Should</strong> return negative if this attribute has lower
	 *      attribute id than argument
	...
	public static class DefaultComparator implements Comparator<PersonAttribute>, Serializable {
		...
		public int compare(PersonAttribute pa1, PersonAttribute pa2) {
			int retValue;
			if ((retValue = OpenmrsUtil.compareWithNullAsGreatest(pa1.getAttributeType(),
					pa2.getAttributeType())) != 0) {
				return retValue;
			}
