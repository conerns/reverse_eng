...
/**
 * This class provides the AOP around each save, (un)void, and (un)retire method
 * in the service
 * layer so that the required data (like creator, dateChanged, dateVoided, etc)
 * can be set
 * automatically and the developer doesn't have to worry about doing it
 * explicitly in the service
 * impl method. <br>
 * <br>
 * See /metadata/api/spring/applicationContext-service.xml for the mapping of
 * this bean. <br>
 * <br>
 * For an Openmrs Service to use this AOP advice class and take advantage of its
 * automatic variable
 * setting, it must have "&lt;ref local="requiredDataInterceptor"/&gt;" in its
 * "preInterceptors".<br>
 * <br>
 * By default, this should take care of any child collections on the object
 * being acted on. Any
 * child collection of {@link OpenmrsObject}s will get "handled" (i.e., void
 * data set up, save data
 * set up, or retire data set up, etc) by the same handler type that the parent
 * object was handled
 * with.<br>
 * <br>
 * To add a new action to happen for a save* method, create a new class that
 * extends
 * {@link RequiredDataHandler}. Add any <b>unique</b> code that needs to be done
 * automatically
 * before the save. See {@link ConceptNameSaveHandler} as an example. (The code
 * should be
 * <b>unique</b> because all other {@link SaveHandler}s will still be called
 * <i>in addition to</i>
 * your new handler.) Be sure to add the {@link org.openmrs.annotation.Handler}
 * annotation (like
 * "@Handler(supports=YourPojoThatHasUniqueSaveNeeds.class)") to your class so
 * that it is picked up
 * by Spring automatically.<br>
 * <br>
 * To add a new action for a void* or retire* method, extend the
 * {@link VoidHandler}/
 * {@link RetireHandler} class and override the handle method. Do not call
 * super, because that code
 * would then be run twice because both handlers are registered. Be sure to add
 * the
...
public class RequiredDataAdvice implements MethodBeforeAdvice {
	...
	public void before(Method method, Object[] args, Object target) throws Throwable {
		...
		// on method calls within a class, only on calls to methods from external
		// classes to methods
		// "update" is not an option here because there are multiple methods that start
		// with "update" but is
		// not updating the primary argument. eg:
		...
		if (methodName.startsWith("save") || methodName.startsWith("create")) {
			...
			// if the first argument is a list of openmrs objects, handle them all now
				...
				// but this only occurs once in the API
				// (AdministrationService.saveGlobalProperties
			...
			// fail early if the method name is not like retirePatient or retireConcept when
			// dealing
	 ...
	 * {@link #recursivelyHandle(Class, OpenmrsObject, User, Date, String, List)}.
	 * Calls that method with the current user and the current Date.
	 *
	 * @param <H>           the type of Handler to get (should extend
	 *                      {@link RequiredDataHandler})
	 * @param handlerType   the type of Handler to get (should extend
	 *                      {@link RequiredDataHandler})
	 * @param openmrsObject the object that is being acted upon
	 * @param reason        an optional second argument that was passed to the
	 *                      service method (usually a
	 *                      void/retire reason)
	...
	/**
	 * This loops over all declared collections on the given object and all declared
	 * collections on
	 ...
	 * @param other          an optional second argument that was passed to the
	 *                       service method (usually a
	 *                       void/retire reason)
	 * @param alreadyHandled an optional list of objects that have already been
	 *                       handled and should
	 *                       not be processed again. this is intended to prevent
	...
	public static <H extends RequiredDataHandler> void recursivelyHandle(Class<H> handlerType,
		...
		// loop over all child collections of OpenmrsObjects and recursively save on
		// those
	...
	/**
	 * This method gets a child attribute off of an OpenmrsObject. It usually uses
	 * the getter for
	 * the attribute, but can use the direct field (even if its private) if told to
	 * by the
	 ...
	 * @return the actual collection of objects that is on the given
	 *         <code>openmrsObject</code>
	 *         <strong>Should</strong> get value of given child collection on given
	 *         field
	 *         <strong>Should</strong> should be able to get annotated private
	 *         fields
	...
	/**
	 * Checks the given {@link Class} to see if it A) is a
	 * {@link Collection}/{@link Set}/
	 ...
	 *         <strong>Should</strong> return false if collection is empty
	 *         regardless of type held
	 */
	...
	/**
	 * Checks if the given field is annotated with a @DisableHandler annotation to
	 * specify
	...
	protected static boolean isHandlerMarkedAsDisabled(Class<? extends RequiredDataHandler> handlerType, Field field) {
		...
		if (!field.isAnnotationPresent(DisableHandlers.class)) {
			...
			// otherwise we need to see if the handler type is one of the types specified in
			// the annotation
